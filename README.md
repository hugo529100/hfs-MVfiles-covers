cool enough ~~~~ no need to say more


---

This plugin needs to be used together with the **MODED-unsupported-videos** plugin, because MODED-unsupported-videos will extract cover images when you open music or video files and store them in the `\cache\videothumbnail` folder inside the corresponding directory.

When browsing the frontend, if you're navigating through a large folder and feel that it's consuming too much memory, you can temporarily disable it in the options panel.

If the thumbnails are stored on a mechanical hard drive, it may cause excessive read operations. In that case, I recommend configuring the settings in the backend control panel to store all thumbnails on an SSD instead. This will greatly improve performance and prevent the mechanical drive from slowing down.

Of course, these features require a bit of trial and error to get used to, but at least for now, it's stable and quite satisfying.

Recommended companion plugins:

- hfs-click-icon-to-show-main
- Orange-theme-main
- Hide-file
- Frontend-performance-optimizer
- File-icon-fix
- thumbnails-main
- sharp-main

---

这份插件需要配搭MODED-unsupported-videos插件进行使用，因为MODED-unsupported-videos会在你打开音乐或视频文件的同时进行封面图的提取，并且贮存在对应文件夹的\cache\videothumbnail内。

在前端页面浏览的时候 如果浏览巨大文件夹的时候，觉得占用页面内存资源，可以在options面板内进行临时性关闭。

如果缩略图放置在机械硬盘内，也许会引起疯狂读取，那么我建议你在后台控制面板内设置自定义，集中所有的缩略图放置在固态硬盘上，那么这样就会大幅提高，也不会引起机械硬盘的性能下降。

当然了，这些功能需要稍微摸索一下，至少目前是比较稳定而且比较满意。

建议配搭插件：
hfs-click-icon-to-show-main
Orange-theme-main
Hide-file
Frontend-performance-optimizer
File-icon-fix

thumbnails-main
sharp-main

https://github.com/user-attachments/assets/f6b83635-e37c-4658-ab8f-fcf57fd1abb9

